...
SpotLightTextureShader::SpotLightTextureShader(Texture* texture, LPCTSTR vsName, LPCTSTR psName): TextureShader(texture, vsName, psName)
{
}

SpotLightTextureShader::~SpotLightTextureShader()
{
...
void SpotLightTextureShader::init(ID3D11Device* device)
{
...
void SpotLightTextureShader::createLightBuffer(ID3D11Device* device)
{
	...
	if (FAILED(device->CreateBuffer(&desc, nullptr, &_d3dLightBuffer))) {
		throw new std::exception("Failed to create light buffer. SpotLightTextureShader.cpp:createLightBuffer()");
	}
}

void SpotLightTextureShader::setShaderParameters(ID3D11DeviceContext* deviceContext)
{
