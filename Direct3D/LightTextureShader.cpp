...
LightTextureShader::LightTextureShader(Texture* texture, LPCTSTR vsName, LPCTSTR psName): TextureShader(texture, vsName, psName)
{
}
...
void LightTextureShader::init(ID3D11Device* device)
{
...
void LightTextureShader::createLightBuffer(ID3D11Device* device)
{
	...
	if (FAILED(device->CreateBuffer(&desc, nullptr, &_d3dLightBuffer))) {
		throw new std::exception("Failed to create light buffer. LightTextureShader.cpp:createLightBuffer()");
	}
}

void LightTextureShader::setShaderParameters(ID3D11DeviceContext* deviceContext)
{
